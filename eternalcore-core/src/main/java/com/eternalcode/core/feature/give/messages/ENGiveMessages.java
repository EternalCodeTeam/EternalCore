...
public class ENGiveMessages extends OkaeriConfig implements GiveMessages {
    ...
    public Notice received = Notice.chat("<green>► <white>You have received: <green>{ITEM}");

    @Comment({" ", "# {PLAYER} - Name of item receiver, {ITEM} - the item"})
    public Notice given = Notice.chat("<green>► <white>Player <green>{PLAYER} <white>has received <green>{ITEM}");

    public Notice noSpace = Notice.chat("<red>✘ <dark_red>Not enough space in inventory!");

    @Comment(" ")
    public Notice invalidItem = Notice.chat("<green>► <white>Not a valid obtainable item!");
}

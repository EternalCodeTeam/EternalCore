...
@LiteArgument(type = int.class, name = EnchantLevelArgument.KEY)
class EnchantLevelArgument extends AbstractViewerArgument<Integer> {

    private static final List<Integer> suggestions = List.of(1, 2, 3, 4, 5);
    static final String KEY = "enchant-level";

    @Inject
    EnchantLevelArgument(TranslationManager translationManager) {
    ...
    public ParseResult<Integer> parse(Invocation<CommandSender> invocation, String argument, Translation translation) {
        try {
            ...
            return ParseResult.failure(translation.enchant().enchantmentLevelUnsupported());
        }
    }

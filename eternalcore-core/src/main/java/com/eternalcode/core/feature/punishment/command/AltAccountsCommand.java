...
class AltAccountsCommand {
    ...
    void execute(@Sender CommandSender operator, @Arg OfflinePlayer target) {
        try {
            List<AltAccount> alts = this.playerIpService.findAltAccounts(target.getUniqueId());
            ...
            this.logger.log(Level.SEVERE, "Failed to look up alt accounts for " + target.getName(), exception);
